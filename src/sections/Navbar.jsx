...
export function Navbar() {
    ...
    const handleDownloadResume = () => {
        ...
        link.href = resumeUrl
        link.download = 'FlexyDev_Resume.pdf'
        ...
        <>
            ...
            <nav
                className={`fixed top-1 w-full z-50 transition-all duration-300 ${
                    scrolled ? 'bg-white dark:bg-zinc-950 backdrop-blur-md border-b border-zinc-800' : 'bg-transparent'
                }`}
            >
                        ...
                        <div className="hidden md:flex items-center gap-8">
                            {navItems.map((item) => (
                                <a
                                    key={item.name}
                                    href={item.href}
                                    className="text-m font-medium text-zinc-900 dark:text-zinc-300 hover:text-white transition-colors relative group"
                                >
                                    {item.name}
